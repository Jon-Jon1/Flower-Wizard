...
document.addEventListener("keydown", (e) => {
	...
	if (
		...
		if (
			keyPressTime[key] &&
			now - keyPressTime[key] < douplePressThreshold
		) {
			player.dashing = true;
...
redFireSheet.src = "Sprites/fire/FireEnemy-Red.png";

let blueFireSheet = new Image();
blueFireSheet.src = "Sprites/fire/FireEnemy-Blue.png";

let purpleFireSheet = new Image();
purpleFireSheet.src = "Sprites/fire/FireEnemy-Purple.png";

let greenFireSheet = new Image();
greenFireSheet.src = "Sprites/fire/FireEnemy-Green.png";

let blackFireSheet = new Image();
blackFireSheet.src = "Sprites/fire/FireEnemy-Black.png";

let whiteFireSheet = new Image();
whiteFireSheet.src = "Sprites/fire/FireEnemy-White.png";
...
function drawGame() {
	...
	if (
		...
		rightSheet.complete &&
		rightSheet.naturalHeight !== 0
	) {
		sheet = rightSheet;
	} else if (
		player.direction === "left" &&
		leftSheet.complete &&
		leftSheet.naturalHeight !== 0
	) {
		sheet = leftSheet;
	} else if (
		player.direction === "up" &&
		upSheet.complete &&
		upSheet.naturalHeight !== 0
	) {
		sheet = upSheet;
	} else if (
		player.direction === "down" &&
		downSheet.complete &&
		downSheet.naturalHeight !== 0
	) {
		sheet = downSheet;
	} else if (
		player.direction === "upRight" &&
		upRightSheet.complete &&
		upRightSheet.naturalHeight !== 0
	) {
		sheet = upRightSheet;
	} else if (
		player.direction === "upLeft" &&
		upLeftSheet.complete &&
		upLeftSheet.naturalHeight !== 0
	) {
		sheet = upLeftSheet;
	} else if (
		player.direction === "downRight" &&
		downRightSheet.complete &&
		downRightSheet.naturalHeight !== 0
	) {
		sheet = downRightSheet;
	} else if (
		player.direction === "downLeft" &&
		downLeftSheet.complete &&
		downLeftSheet.naturalHeight !== 0
	) {
		sheet = downLeftSheet;
	} else if (
		player.direction === "idle" &&
		idleSheet.complete &&
		idleSheet.naturalHeight !== 0
	) {
		sheet = idleSheet;
	}
	...
	for (let i = 0; i < playerProjectiles.length; i++) {
		...
		gameCtx.beginPath();
		gameCtx.arc(
			playerProjectiles[i].x + 1.5,
			playerProjectiles[i].y + 3,
			5,
			0,
			Math.PI * 2,
		);
	...
	if (orbSheet && orbSheet.complete && orbSheet.naturalHeight !== 0) {
		...
		gameCtx.drawImage(
			...
			orbFrameWidth,
			orbFrameHeight,
			orbX - orbFrameWidth / 2,
			orbY - orbFrameHeight / 2,
			orbFrameWidth,
			orbFrameHeight,
		);
	} else {
...
function playerMovement() {
	...
	if (player.dashing) {
		...
		if (player.dashDirection === "arrowup") {
			player.y -= dashMoveAmount;
		} else if (player.dashDirection === "arrowdown") {
			player.y += dashMoveAmount;
		} else if (player.dashDirection === "arrowleft") {
			player.x -= dashMoveAmount;
		} else if (player.dashDirection === "arrowright") {
			player.x += dashMoveAmount;
		}
	...
	if (player.dead == false) {
		...
		if (keys["arrowup"] && keys["arrowdown"] && keys["arrowright"]) {
			player.direction = "right";
		}
	}
	if (!gameStarted) {
...
function spawnEnemy(count) {
	for (let i = 0; i < count; i++) {
		enemies.push({
			x: Math.random() * gameCtx.width - 100 + 50,
			y: -254,
			fallSpeed: 5,
			health: 50,
		});
...
function col(entity, colType) {
	if (entity == "player") {
		if (colType == "wall") {
			...
			if (player.x + 204 > gameCanvas.width)
				player.x = gameCanvas.width - 204;
			if (player.y + 254 > gameCanvas.height)
				player.y = gameCanvas.height - 254;
		}
	}
	if (entity == "playerProj") {
		if (colType == "wall") {
			for (let i = 0; i < playerProjectiles.length; i++) {
				...
				if (
					element.x > gameCanvas.width ||
					element.x < 0 ||
					element.y > gameCanvas.height ||
					element.y < 0
				) {
